...
const getAllProductsCreated = catchAsync(async (req, res, next) => {
  ...
  const products = await Product.findOne({ where: { userId: sessionUser.id } });

  if (!products) {
    return next(new AppError("Products Not Found", 404));
  }

  // let dataMessage = products || "You haven't registered any product yet";

  // if (!products === true) {
  //   dataMessage = "You haven't registered any product yet";
  // } else {
  //   dataMessage = products;
  // }
...
const getPurchasesMade = catchAsync(async (req, res, next) => {
  const { sessionUser } = req;
...
const getPurchasesMadeById = catchAsync(async (req, res, next) => {
  const { sessionUser } = req;
  const { order } = req;
